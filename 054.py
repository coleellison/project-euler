...
rawinp = """8C TS KC 9H 4S 7D 2S 5D 3S AC
...
p2 = []
for deal in temp:
    curr = [[card[0], card[1]] for card in deal]
    for idx in range(len(curr)):
        if curr[idx][0].isnumeric():
            curr[idx][0] = int(curr[idx][0])
        else:
            curr[idx][0] = facedict[curr[idx][0]]
    p1.append(curr[:5])
    p2.append(curr[5:])
...
def score(hand): #check for hand type in descending order (ensures highest score possible)
    if royalflush(hand):
